...
export default function ResumeTailoring() {
  ...
  async function analyze(e) { e.preventDefault(); setError(''); if (!resumeId || !jobDescription.trim()) { setError('Select an original resume and paste a job description.'); return } try { setBusy(true); setAnalysis((await analyzeTailoring({ resumeId: Number(resumeId), jobDescription })).data) } catch (e) { setError('Could not analyze this resume.'); console.error(e) } finally { setBusy(false) } }
