...
export default function Profile() {
  ...
  React.useEffect(() => {
    ...
    if (gmailParam === 'connected') { setGmailMessage('Gmail connected successfully.'); setGmailMessageIsError(false) }
    else if (gmailParam === 'error') { setGmailMessage('Gmail authorization failed. Please try connecting again.'); setGmailMessageIsError(true) }
    const next = new URLSearchParams(searchParams); next.delete('gmail'); setSearchParams(next, { replace: true })
  }, [searchParams, setSearchParams])
  async function connectGmail() {
    try { const result = await beginGmailConnect(); window.location.href = result.authorizationUrl }
    catch (e) { setGmailMessage(e.response?.data?.error || 'Could not start Gmail connection.'); setGmailMessageIsError(true) }
  }
  async function sync() { try { setGmailBusy(true); const result = await syncGmail(); setReviewQueue(await getGmailReviewQueue()); setGmailMessage(`${result.processed} new messages processed.`); setGmailMessageIsError(false) } catch (e) { setGmailMessage(e.response?.data?.error || 'Gmail sync failed.'); setGmailMessageIsError(true) } finally { setGmailBusy(false) } }
  async function disconnect() { try { setGmailBusy(true); await disconnectGmail(); setGmail({ connected: false }); setGmailMessage('Gmail disconnected.'); setGmailMessageIsError(false) } catch (e) { setGmailMessage(e.response?.data?.error || 'Could not disconnect Gmail.'); setGmailMessageIsError(true) } finally { setGmailBusy(false) } }
  ...
  return (
    <Layout title="Profile" subtitle="Your account details">
      <div className="max-w-md bg-surface border border-line rounded-xl2 shadow-card p-6">
          ...
          {interviewCandidates.length === 0 ? <p className="text-xs text-muted">No pending interview candidates right now.</p> : interviewCandidates.map(candidate => (
            <div key={`${candidate.source}-${candidate.id}`} className="border border-line rounded-lg p-3 mb-2">
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button onClick={() => { const applicationId = document.getElementById(`interview-app-${candidate.source}-${candidate.id}`).value; if (!applicationId) return; confirmCandidate(candidate, applicationId, 'INTERVIEW') }} className="px-3 py-2 rounded-full bg-accent text-white hover:bg-accent-dark text-xs font-medium">Confirm interview</button>
              </div>
            </div>
          ))}
        </section>}
        ...
        <button
          onClick={logout}
          className="w-full inline-flex items-center justify-center gap-1.5 bg-status-rejected text-white text-sm font-medium py-2.5 rounded-full hover:opacity-90"
        >
