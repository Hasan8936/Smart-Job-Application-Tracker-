    ...
    <div className="grid lg:grid-cols-3 gap-6">
      <section className="bg-surface border border-line rounded-xl2 shadow-card p-5 h-fit">
        ...
        <form onSubmit={analyze} className="space-y-3">
          ...
          <button disabled={busy} className="w-full inline-flex items-center justify-center gap-1.5 btn-gradient text-sm font-medium px-4 py-2.5 rounded-full disabled:opacity-50"><Sparkles size={15} />{busy ? 'Analyzing...' : 'Generate suggestions'}</button>
        </form>
      ...
      <section className="lg:col-span-2 space-y-6">
        {!analysis ? <div className="bg-surface border border-dashed border-line rounded-xl2 p-10 text-center text-sm text-muted">Your grounded suggestions will appear here for review.</div> : <>
          ...
          <button onClick={createVersion} disabled={busy || !analysis.suggestions.some(item => item.decision === 'ACCEPTED')} className="inline-flex items-center gap-1.5 btn-gradient text-sm font-semibold px-4 py-2.5 rounded-full disabled:opacity-50"><FileText size={15} />Create new resume version</button>
        </>}
