          ...
          {interviewCandidates.length === 0 ? <p className="text-xs text-muted">No pending interview candidates right now.</p> : interviewCandidates.map(candidate => (
            <div key={`${candidate.source}-${candidate.id}`} className="border border-line rounded-lg p-3 mb-2">
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button onClick={() => { const applicationId = document.getElementById(`interview-app-${candidate.source}-${candidate.id}`).value; if (!applicationId) return; confirmCandidate(candidate, applicationId, 'INTERVIEW') }} className="px-3 py-2 rounded-full btn-gradient text-xs font-medium">Confirm interview</button>
              </div>
            </div>
          ))}
        </section>}
