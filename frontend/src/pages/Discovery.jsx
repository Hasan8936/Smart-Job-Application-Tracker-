...
export default function Discovery() {
  ...
  return <Layout title="Discover jobs" subtitle="Find roles from configured official job sources">
    <form onSubmit={submit} className="bg-surface border border-line rounded-xl2 p-4 mb-5 shadow-card">
      <div className="flex flex-col sm:flex-row gap-2"><div className="relative flex-1"><Search size={16} className="absolute left-3 top-3 text-muted" /><input value={filters.q} onChange={e => setFilters({ ...filters, q: e.target.value })} placeholder="Search title or company" className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-line bg-paper" /></div><button className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-full bg-accent text-accent-ink text-sm font-medium hover:bg-accent-dark"><Search size={15} /> Search</button></div>
    ...
    <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><SlidersHorizontal size={16} className="text-muted" /><h2 className="font-display text-lg">Available jobs</h2></div><div className="flex items-center gap-3">{newJobsCount > 0 && <button onClick={() => { setShowingOnlyNew(current => !current); setPage(0) }} className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium ${showingOnlyNew ? 'bg-accent text-accent-ink' : 'border border-line text-ink-soft'}`}><Sparkles size={14} /> {showingOnlyNew ? 'Showing new' : `${newJobsCount} new`}</button>}<span className="text-sm text-muted">{jobs.totalElements || 0} roles</span><button onClick={syncSources} disabled={syncing} className="inline-flex items-center gap-2 px-3 py-2 rounded-full border border-line text-ink-soft text-sm font-medium disabled:opacity-50">{syncing ? <Loader2 size={15} className="animate-spin" /> : <RefreshCw size={15} />} {syncing ? 'Syncing…' : 'Sync sources'}</button></div></div>
