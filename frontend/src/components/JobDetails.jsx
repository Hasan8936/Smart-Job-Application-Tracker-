  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-5">
      ...
      <section className="relative w-full sm:max-w-2xl max-h-[90vh] overflow-y-auto bg-surface sm:rounded-xl2 shadow-card p-5 sm:p-7">
        <button onClick={onClose} className="absolute top-4 right-4 h-10 w-10 rounded-lg border border-line flex items-center justify-center" aria-label="Close"><X size={17} /></button>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          <a href={job.applyUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-ink text-white rounded-lg px-4 py-2.5 text-sm font-medium"><ExternalLink size={15} /> Open official application</a>
          {onGenerate && <button onClick={() => onGenerate('COVER_LETTER')} className="inline-flex items-center gap-2 border border-line rounded-lg px-3 py-2.5 text-sm"><FileText size={15} /> Cover letter</button>}
          {onGenerate && <><button onClick={() => onGenerate('COLD_EMAIL')} className="inline-flex items-center gap-2 border border-line rounded-lg px-3 py-2.5 text-sm"><Mail size={15} /> Cold email</button>
          <button onClick={() => onGenerate('INTERVIEW_QUESTIONS')} className="inline-flex items-center gap-2 border border-line rounded-lg px-3 py-2.5 text-sm"><MessageSquare size={15} /> Interview questions</button>
          <button onClick={() => onGenerate('IMPROVE_RESUME')} className="inline-flex items-center gap-2 border border-line rounded-lg px-3 py-2.5 text-sm"><Pencil size={15} /> Improve resume</button></>}
        </div>
...
function SkillGroup({ label, skills }) {
  return <div className="mt-5"><h3 className="font-display text-base mb-2">{label}</h3><div className="flex flex-wrap gap-1.5">{skills.map(skill => <span key={skill} className="px-2 py-1 rounded-md bg-accent/20 text-ink text-xs">{skill}</span>)}</div></div>
}